...
$sql = "SELECT id, titolo, foto, voto, user_id FROM recommendation_manga ORDER BY id DESC";
...
<main>
    <div style="max-width:900px; margin: 40px auto 24px; padding: 0 24px; display:flex; align-items:center; justify-content:space-between;">
        <div>
            ...
            <h2 style="margin-bottom:0;">Manga</h2>
        </div>
        <?php if(isset($_SESSION["user_id"])): ?>
            <a href="aggiungiManga.php" class="btn">+ Aggiungi consiglio</a>
        <?php else: ?>
    ...
    <div class="card-grid">
        <?php if($result && $result->num_rows > 0): ?>
                ...
                <div class="card">
                    ...
                    <?php if(isset($_SESSION["user_id"]) && $_SESSION["user_id"] == $row["user_id"]): ?>
                        <form method="POST" action="elimina.php" onsubmit="return confirm('Sei sicuro di voler eliminare questo consiglio?');" style="margin-top:12px;">
                            ...
                            <input type="hidden" name="tabella" value="recommendation_manga">
                            <input type="hidden" name="ritorna" value="manga.php">
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <p class="text-muted" style="padding: 0 24px;">Nessun manga consigliato ancora.</p>
        <?php endif; ?>
    </div>
</main>
