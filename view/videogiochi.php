...
$sql = "SELECT id, titolo, foto, voto, user_id FROM recommendation_videogioco ORDER BY created_at DESC";
...
<head>
    ...
    <title>Videogiochi — RecWorld</title>
...
<main>
    <div style="max-width:900px; margin: 40px auto 24px; padding: 0 24px; display:flex; align-items:center; justify-content:space-between;">
        <div>
            ...
            <h2 style="margin-bottom:0;">Videogiochi</h2>
        </div>
        <?php if(isset($_SESSION["user_id"])): ?>
            <a href="aggiungiVideogioco.php" class="btn">+ Aggiungi consiglio</a>
        <?php else: ?>
        ...
        <?php if($result && $result->num_rows > 0): ?>
            <?php while($row = $result->fetch_assoc()): ?>
                <div class="card">
                    ...
                    <span class="badge badge-game">Videogioco</span>
                    ...
                    <?php if(isset($_SESSION["user_id"]) && $_SESSION["user_id"] == $row["user_id"]): ?>
                        <form method="POST" action="elimina.php" onsubmit="return confirm('Sei sicuro di voler eliminare questo consiglio?');" style="margin-top:12px;">
                            ...
                            <input type="hidden" name="tabella" value="recommendation_videogioco">
                            <input type="hidden" name="ritorna" value="videogiochi.php">
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <p class="text-muted" style="padding: 0 24px;">Nessun videogioco consigliato ancora.</p>
        <?php endif; ?>
    </div>
</main>
